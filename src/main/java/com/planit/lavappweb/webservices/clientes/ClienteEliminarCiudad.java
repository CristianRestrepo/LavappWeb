 ...
 * USAGE:
 * <pre>
 *        ClienteEliminarCiudad client = new ClienteEliminarCiudad();
 ...
 */
public class ClienteEliminarCiudad {
    private WebTarget webTarget;
    private Client client;
    private static final String BASE_URI = "http://localhost:8084/LavappService/webresources/";

    public ClienteEliminarCiudad() {
        client = ResteasyClientBuilder.newBuilder().build();
        webTarget = client.target(BASE_URI).path("eliminarCiudad");
    }
     ...
     */
    public <T> T eliminarCiudad(Class<T> responseType, String idCiudad) throws ClientErrorException {
